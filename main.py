...
stockfish.update_engine_parameters(
    {
        "Hash": config["engine"]["hash"],
        "Minimum Thinking Time": config["engine"]["min_time"],
    }
)
...
def SeleniumFunction():
    ...
    PLAYER_COLOR = None
    white_king = utils.find_by_css_selector_persist(
        driver, 'div[class="piece wk square-51"]', wait=5
    )
    black_king = utils.find_by_css_selector_persist(
        driver, 'div[class="piece bk square-58"]', wait=5
    )
